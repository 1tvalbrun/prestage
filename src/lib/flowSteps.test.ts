...
test("the rail shows the lane's pre-read step, named by the lane, when the practice has prep", () => {
  assert.deepEqual(labels(flowSteps(PACKS.founder, {})), ["Brief", "Pre-read", "Counterpart", "Room"])
  assert.deepEqual(labels(flowSteps(PACKS.interview, {})), ["Brief", "Blueprint", "Counterpart", "Room"])
...
test("a no-prep practice drops the pre-read step, and the read/audit stages with it", () => {
  ...
  assert.deepEqual(labels(steps), ["Brief", "Counterpart", "Room"])
...
test("before the pack is known the rail assumes the full four steps", () => {
  assert.deepEqual(labels(flowSteps(null, {})), ["Brief", "Pre-read", "Counterpart", "Room"])
})
