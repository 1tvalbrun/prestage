...
// RoomShell's credentials-cache workaround (the per-attempt connectUrl on
// AvatarProvider) rests on two facts about @runwayml/avatars-react
// internals. The first workaround died silently when a refactor stopped
// exercising them — observed live 2026-08-28 as "Go again" joining the
// previous session's dead room. These assertions make an SDK upgrade that
// changes either fact fail the suite instead of breaking same-persona
// re-entry in production.
